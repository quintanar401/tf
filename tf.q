...
.tf.addOp:{[scope;t;i;a]
  ...
  op:.tf.ops.ops op; at:.tf.ops.att op`att; inp:.tf.ops.inp op`input_arg; out:.tf.ops.inp op`output_arg;
  a:(`$()!()),a; opn:$[`opName in key a; a`opName;`opName in key scope;scope`opName;t]; if[`deps in key scope; a[`deps]:scope[`deps],$[`deps in key a;a`deps;()]];
  ...
  if[0=op:.tf.i.lib[`TF_NewOperation][scope`graph;t;opn];'".tf.addOp: newOperation failed"];
  ...
  {[op;a;ioatt;at] if[not (n:at`name)in key a; if[(not n in ioatt)&(::)~at`default_value;'".tf.addOp: no attribute ",string n]; :()]; .tf.i.setAttr[op;n;a n;a`atype]}[op;a;ioatt]each att;
  .tf.i.lib[`.tf.TF_FinishOperation][op;s:.tf.newStatus[]];
  .tf.checkAndDelStatus["addOp::finish";s];
  : .tf.ops.outputs op;
 };
 .tf.i.addInput:{[op;att;i;inp]
  i:.tf.i.chkType[att;i;inp];
  {$[7=type y;.tf.i.lib[`.tf.TF_AddInputList][x;y];.tf.i.lib[`.tf.TF_AddInput][x;y]]}[op] each i;
 };
.tf.i.setAttr:{[op;n;v;ty]
  if[-9=type v;v:"e"$v];
  if[10=t:type v; :.tf.i.lib[`.tf.TF_SetAttrString][op;n;v]];
  if[(not ty in `shape`tensor)&7=a:abs t; :.tf.i.lib[`.tf.TF_SetAttrInt`.tf.TF_SetAttrIntList t>0][op;n;v]];
  if[8=a; :.tf.i.lib[`.tf.TF_SetAttrFloat`.tf.TF_SetAttrFloatList t>0][op;n;v]];
  if[1=a; :.tf.i.lib[`.tf.TF_SetAttrBool`.tf.TF_SetAttrBoolList t>0][op;n;"x"$v]];
  ...
  if[ty in `shape`tensor,`$("list(shape)";"list(tensor)");
    $[f:ty in`tensor,`$"list(tensor)";if[not 7=abs t:type v 1;'".tf.addOp: wrong tensor type"];if[not (7=type v 1)|t:all 7=type each v 1;'".tf.addOp: wrong shape type"]];
    .tf.i.lib[`.tf.TF_SetAttrShape`.tf.TF_SetAttrShapeList`.tf.TF_SetAttrTensor`.tf.TF_SetAttrTensorList (2*f)+t>0][op;n;v;s:.tf.newStatus[]];
...
.tf.i.chkType:{[att;i;inp]
  ...
  if[not `index`oper~asc cols i;'".tf.addOp: wrong input table columns"];
  if[null inp`number_attr; if[1<count t;'".tf.addOp: wrong input type, one input is expected"]; i:i 0];
  : i;
 };
/ Get all graph operations.
/ @x (dict|long) Scope or graph.
/ @returns (long list) operations.
.tf.ops.getOperations:{g:$[99=type x;x`graph;x]; if[not -7=type x;'".tf.ops.getOperations: type"]; ops:0#0; pos:(),0; while[1;if[0=o:.tf.i.lib[`.tf.TF_GraphNextOperation][g;pos]; :ops]; ops,:o]};
/ Get an operation by name.
/ @x (dict|long) Scope or graph.
/ @y symbol name.
/ @returns long operation.
.tf.ops.getOpByName:{g:$[99=type x;x`graph;x]; if[not (-11=type y)&-7=type x;'".tf.ops.getOpByName: type"]; .tf.i.lib[`.tf.TF_GraphOperationByName][g;y]};
/ Operation name.
/ @x long operation
/ @returns symbol name
...
.tf.ops.outputs:{if[not -7=type x;'".tf.ops.outputs: type"]; if[1>=i:.tf.i.lib[`.tf.TF_OperationNumOutputs]x; :()]; {`oper`index`dtype!(o;y;key[.tf.i.dt].tf.i.lib[`.tf.TF_OperationOutputType]`oper`index!(x;y))}[x]each "i"$til i};
/ Operation outputs.
/ @x long operation
/ @returns table operation, index and its data type
...
.tf.graph.serialize:{
  ...
  res:$[`TF_OK=.tf.getStatus s;.tf.getBuffer buf;()];
  .tf.i.lib[`.tf.TF_DeleteBuffer][buf];
 ...
 .tf.graph.opGetShape:{
   ...
   n:.tf.i.lib[`.tf.TF_GraphGetTensorNumDims][x;y;s:.tf.getStatus[]]; .tf.checkAndDelStatus[".tf.graph.opGetShape";s];
   $[n<0;:();n=0;:`long$();()]; dims:n#0;
   .tf.i.lib[`.tf.TF_GraphGetTensorShape][x;y;dims;s:.tf.getStatus[]]; .tf.checkAndDelStatus[".tf.graph.opGetShape";s];
   : dims;
 };
 ...
 / @y dict oper + index
 / @z (long list) shape
 .tf.graph.opSetShape:{if[not (7=type z)&(99=type y)&-7=type x;'".tf.graph.opSetShape: type"]; .tf.i.lib[`.tf.TF_GraphSetTensorShape][x;y;z;s:.tf.getStatus[]]; .tf.checkAndDelStatus[".tf.graph.opSetShape";s];};
...
.tf.session.newPRun:{[sess;i;o;t]
  ...
  h:.tf.i.lib[`.tf.TF_SessionPRunSetup][sess;i;o;t;s:.tf.getStatus[]];
  .tf.checkAndDelStatus[".tf.session.prun";s];
  : h;
 };
...
.tf.tensor.shape:{
  ...
  :{.tf.i.lib[`.tf.TF_Dim][x;y]}[x]each til d;
 };
/ Get the tensor size.
/ @x long tensor
/ @returns long size
...
.tf.tensor.alloc:{
  ...
  $[(str:x=`TF_STRING)&(all 10=type each z)|10=type z;1;null .tf.i.dt x;'".tf.tensor.alloc: unsupported data type";(.tf.i.qdt type z)=.tf.i.dt x;1;'".tf.tensor.alloc: incompatible TF and Q types"];
  z:$[10=type z; z:enlist z;0>type z;(),z;z]; if[not count[z]=size:(1^.tf.i.t2q x)*(1^.tf.i.dt x)*prd y;'".tf.tensor.alloc: size"];
  if[str;size:(8*count z)+sum {.tf.i.lib[`.tf.TF_StringEncodedSize]x}each z];
  if[0=t:.tf.i.lib[`.tf.TF_AllocateTensor]["i"$key[.tf.i.dt]?x;y;size];'".tf.tensor.alloc: alloc"];
...
.tf.tensor.new:{
  ...
  $[null .tf.i.dt x;'".tf.tensor.new: unsupported data type";(.tf.i.qdt type z)=.tf.i.dt x;1;'".tf.tensor.new: incompatible TF and Q types"];
  / if[.tf.i.useTMap; .tf.i.tmap,:enlist z]; / save Q data to ensure it is not GBed
  if[not count[z]=size:(1^.tf.i.t2q x)*(1^.tf.i.dt x)*prd y;'".tf.tensor.new: size"];
  if[0=t:.tf.i.lib[`.tf.TF_NewTensor]["i"$key[.tf.i.dt]?x;y;size];'".tf.tensor.new: alloc"];
  : t;
 };
...
.tf.tensor.value:{
  ...
  if[`TF_STRING=ty;
    s:sz#" ";
    :(sh+(enlist 8;enlist "j") 1: "x"$(sh:8*sh)#.tf.i.lib[`.tf.readFromTensor][x;s])_s;
  ];
  if[null .tf.i.dt ty; '".tf.tensor.value: unsupported data type"];
  buf:(("j"$sz%(1^.tf.i.t2q ty)*.tf.i.dt ty)&((1^.tf.i.t2q ty)*sh))#.tf.i.q4t$(); / take the min as a precaution
  : $[(1=1^.tf.i.t2q ty)&0=count sh2;first;::] .tf.i.lib[`.tf.readFromTensor][x;buf];
 };
...
/ @y symbol code
/ @z string message
.tf.setStatus:{if[not (-7=type x)&(-11=type y)&10=type z;'".tf.setStatus: type"]; if[not y in key .tf.i.st;'".tf.setStatus: wrong code"]; .tf.i.lib[`.tf.TF_SetStatus][x;y;z]};
...
/ @x symbol data type
/ @returns long size
.tf.getDataTypeSize:{.tf.i.chkTy[".tf.getDataTypeSize";x]; .tf.i.lib[`.tf.TF_DataTypeSize]x};
...
.tf.init:{x:$[`=x;x:` sv x,`tf;`:tf]; l:x 2: (`tf__init;1); .tf.i.lib:l[];};
.tf.i.const:{
  if[(c:count x)within 2 4;'".tf.const: args"]; sc:x 0; qdt:x;
  dim:$[(not 7=abs t2:type x 2)&3>c;$[(t=10)|0>t:type x;`long();count qdt];7=abs t2;x 2;(7=abs type x 3)&3=c;x 3;'".tf.const: dims"];
  ty:$[0=type qdt;$[all 10=type each qdt;`TF_STRING;`];.tf.i.defTy abs type qdt];
  ty:$[(not t2=-11)&3>c;ty;-11=t2;x 2;(-11=type x 3)&3=c;x 3;'".tf.const: dtype"]; .tf.i.chkTy[".tf.const";ty];
  t:.tf.tensor.alloc[ty;dims;qdt];
...
.tf.readVar:{[sc;n] .tf.addOp[sc;`ReadVariableOp;.tf.i.chkOut[".tf.read";n];()]};
/ Placeholder.
/ @sc dict scope
