.t.testScope:{
  ...
  {c:.tf.const[x;1];if[not y~n:.tf.ops.name c[0]`oper;'"wrong oper name: ",string[y]," vs ",string n]}'[(root;sub1;sub2;sub1a;sub2a);`$("Const";"x/Const";"x_1/Const";"x/y/Const";"x_1/y/Const")];
  .tf.delScope root;
 };
...
.t.testControl:{
  ...
  r:.tf.readVar[s,enlist[`deps]!enlist upd[0]`oper;v];
  sess:.tf.newSession[s;0];
  .tf.session.run[sess;();();enlist init[0]`oper;0;0];
  if[not t~r:{r:.tf.session.run[x;();y;();0;0]; .tf.tensor.value r 0}[sess;read] each t:til 10;'"run result differ: ",.Q.s1 r];
 };
.t.testShape:{
  s:.tf.newScope[];
  input:.tf.placeholder[s;`TF_FLOAT];
  output:.tf.matMul[s,enlist[`transpose_b]!(),1b;.tf.const[s;10 20e;2 1];input];
  0N!.tf.graph.opGetShape[s`graph;output 0];
 };
