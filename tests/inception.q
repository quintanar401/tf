.t.testInception:{
  ...
  if[not (i:`$.z.x 0) in key `:.;'"provide a jpeg as an argument"];
  ...
  o:.tf.session.run[sess;update tensor:t from .tf.ops.getOpByName[g;`input];.tf.ops.getOpByName[g;`output];();0;0];
...
.tst.getImage:{
  t:.tf.tensor.alloc[`TF_STRING;count img;img:read1 x]; / expects a string but in Q2C a byte list should do as well
  sess:.tf.newSession[g;0];
  s:.tf.newScope[];
  inp:.tf.placeholder[s;`TF_STRING];
  op:.tf.addOp[s;`DecodeJpeg;inp;enlist[`channels]!3];
