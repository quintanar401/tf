...
.tst.testSession:{[d]
  ...
  g:.tst.create d 1; / scope, inp, out
...
.t.testSessionString:{
  ...
  hash:.tf.addOp[s;`StringToHashBucketFast;feed;enlist[`min_buckets]!enlist prd 32#2];
...
.t.testSessionPRun:{
  ...
  if[not 3=v:.tf.tensor.value r 0;'"wrong plus2 value(10): ",string v];
  .tf.tensor.del r 0;
